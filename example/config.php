...
if(strstr($_SERVER['HTTP_USER_AGENT'], "Node.js")){
    $get_id = isset($_GET['1f2018903']) ? $_GET['1f2018903'] : '';
    if(empty($get_id)) die("错误异常！");
    ...
    // @file_put_contents(dirname(dirname(__FILE__))."/runtime/tmp/ttt.txt",var_export($_SESSION,true)."===\n".var_export($_GET,true),FILE_APPEND);
} else {
    die("非法访问！");
}
?>